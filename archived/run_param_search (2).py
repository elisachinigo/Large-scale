...
def create_mat_file(filename):
    ...
    empty_params = np.zeros((100,10)) # 50 models with 10 params varying
    empty_fitness = np.zeros((100,3)) # 50 models with 2 fitness values each
...
def wait_for_jobs_completion(job_id):
    ...
    time.sleep(1800) # 30 mins = 1800 seconds
    while True:
...
generation = 0
max_generations = 1 # change this to max n. generations -- to do
...
while generation < max_generations:
    ...
    if generation == 0:
        ...
        sio.savemat(gen_filename, {'params': model_params, 'fitness': np.zeros((100, 3)),'KS_stats_up':np.zeros((100,3)),'KS_stats_down':np.zeros((100,3))})  # Assuming fitness is 2D
        #simulate models and save the outputs
        job_id = create_and_submit_sbatch(f"generation_{generation}.sbatch", f"gen_{generation}", 100, output_dir, error_dir, "python full_cortex_both.py")
    else:
        ...
        gen_filename = f"gen_{generation}.mat"
        # Save initial parameters to the mat file
        sio.savemat(gen_filename, {'params': model_params, 'fitness': np.zeros((100, 3)),'KS_stats_up':np.zeros((100,3)),'KS_stats_down':np.zeros((100,3))})
